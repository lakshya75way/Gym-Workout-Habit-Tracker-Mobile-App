...
import { useWorkoutActions, useWorkoutById } from "../workout.store";
...
export const EditWorkoutScreen = memo(() => {
  const navigation =
    useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const route = useRoute<EditWorkoutRouteProp>();
  const { workoutId } = route.params;
  const workout = useWorkoutById(workoutId);
  ...
  useEffect(() => {
    if (workout) {
      ...
      setVideoUri(workout.video_uri || "");

      const img = workout.image_uri || "";
      if (img.startsWith("http")) {
        setExternalImageUri(img);
      ...
      const days = [];
      const mask = workout.day_mask || 0;
      for (const day of DAYS) {
        if ((mask & day.value) === day.value) {
          days.push(day.value);
        }
      }
  ...
  const handleUpdate = useCallback(async () => {
    ...
    if (!trimmedName) {
      Alert.alert("Required", "Workout name cannot be empty");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Failed", "Could not update the workout blueprint.");
    } finally {
      ...
      >
        ...
        >
          <View style={styles.header}>
            ...
            <Text style={styles.title}>Refine Plan</Text>
            <View style={{ width: 44 }} />
          </View>
